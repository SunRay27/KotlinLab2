...
fun main() {
    ...
    println("Min area shape: ${(collector.getMinAreaShape() as BaseShape)}")
    println("Max area shape: ${(collector.getMaxAreaShape() as BaseShape)}")
    println("Total area: ${(collector.getTotalShapesArea())}")

    println()
    println("White fill color shapes:")
    collector.getAllShapesWithFillColor(white).forEach { println((it as BaseShape).toString()) }

    println()
    println("Red border color shapes:")
    collector.getAllShapesWithBorderColor(red).forEach { println((it as BaseShape).toString()) }
